...
fun ProfileScreen(onSettings: () -> Unit, vm: AppViewModel = viewModel()) {
    ...
    val earnedBadgeIds = badges.map { it.id }.toSet()

    Column(Modifier.fillMaxSize().background(BgDark).verticalScroll(rememberScrollState())) {
        // Header
        Box(Modifier.fillMaxWidth().background(
            Brush.verticalGradient(listOf(Color(0xFF1A1040), BgDark))).padding(24.dp, 48.dp, 24.dp, 24.dp)) {
            Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.fillMaxWidth()) {
                ...
                LinearProgressIndicator(progress = { if (needed>0) earned.toFloat()/needed else 0f },
        ...
        val statItems = listOf(
            ...
            Triple("${earnedBadgeIds.size}", "ব্যাজ অর্জিত", Gold)
        )
        ...
        // Badges
        Text("ব্যাজ সংগ্রহ (${earnedBadgeIds.size}/${badgeDefs.size})",
        ...
        LazyRow(contentPadding = PaddingValues(horizontal = 16.dp), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            items(badgeDefs) { badge ->
                ...
                Card(Modifier.width(100.dp), colors = CardDefaults.cardColors(
                    containerColor = if (earned) Gold.copy(0.1f) else BgCard),
                    ...
                    Column(Modifier.padding(10.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                        Box(Modifier.size(40.dp).background(
                            ...
                            contentAlignment = Alignment.Center) {
                            Text(if (earned) "★" else "○", fontSize = 20.sp,
