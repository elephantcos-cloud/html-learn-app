    ...
    Column(Modifier.fillMaxSize().background(BgDark).padding(top = 48.dp)) {
        ...
        quiz.options.forEachIndexed { idx, option ->
            ...
            Card(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 5.dp)
                ...
                onClick = { if (!showResult) { selected = idx; showResult = false } }) {
                Row(Modifier.padding(14.dp), verticalAlignment = Alignment.CenterVertically) {
                    Box(Modifier.size(28.dp).background(BgElevated, CircleShape),
        ...
        if (showResult) {
            Card(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp),
                colors = CardDefaults.cardColors(containerColor = if (isCorrect) Accent.copy(0.1f) else Error.copy(0.1f)),
                shape = RoundedCornerShape(12.dp)) {
                Row(Modifier.padding(12.dp), verticalAlignment = Alignment.Top) {
                    Icon(if (isCorrect) Icons.Filled.CheckCircle else Icons.Filled.Cancel,
                        null, tint = if (isCorrect) Accent else Error, modifier = Modifier.size(20.dp))
                    Spacer(Modifier.width(8.dp))
                    Column {
                        Text(if (isCorrect) "সঠিক উত্তর!" else "ভুল উত্তর!",
                            fontWeight = FontWeight.Bold, color = if (isCorrect) Accent else Error)
    ...
    Column(Modifier.fillMaxSize().background(BgDark), horizontalAlignment = Alignment.CenterHorizontally,
        ...
        Box(Modifier.size(120.dp).background(color.copy(0.2f), CircleShape),
            ...
            Column(horizontalAlignment = Alignment.CenterHorizontally) {
                Text("$pct%", style = MaterialTheme.typography.headlineLarge, color = color, fontWeight = FontWeight.ExtraBold)
        ...
        Button(onClick = onDone, modifier = Modifier.padding(horizontal = 32.dp).fillMaxWidth().height(52.dp),
            colors = ButtonDefaults.buttonColors(containerColor = Primary),
            shape = RoundedCornerShape(14.dp)) {
