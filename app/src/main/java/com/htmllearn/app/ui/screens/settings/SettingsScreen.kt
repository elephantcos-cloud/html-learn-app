...
@Composable
fun SettingsScreen(onBack: () -> Unit, vm: AppViewModel = viewModel()) {
        ...
        Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp),
            ...
            Card(colors = CardDefaults.cardColors(BgCard), shape = RoundedCornerShape(14.dp)) {
                Column(Modifier.padding(16.dp)) {
                    ...
                    Slider(value = goalMinutes.toFloat(), onValueChange = { goalMinutes = it.toInt() },
                ...
                Column(Modifier.padding(16.dp)) {
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Column(Modifier.weight(1f)) {
                            ...
                            Text("প্রতি ৩ ঘণ্টায় reminder পাবে", style = MaterialTheme.typography.bodySmall, color = TextHint)
                        }
                        Switch(checked = notifEnabled, onCheckedChange = { notifEnabled = it },
                            colors = SwitchDefaults.colors(checkedThumbColor = Warning, checkedTrackColor = Warning.copy(0.4f)))
                    }
                    if (notifEnabled) {
                        Spacer(Modifier.height(12.dp))
                        Text("মূল reminder সময়: $reminderHour:00", style = MaterialTheme.typography.bodySmall)
                        Slider(value = reminderHour.toFloat(), onValueChange = { reminderHour = it.toInt() },
            ...
            Card(colors = CardDefaults.cardColors(BgCard), shape = RoundedCornerShape(14.dp)) {
                Column(Modifier.padding(16.dp)) {
                    Text("Notification পরীক্ষা", style = MaterialTheme.typography.titleMedium, color = Accent)
                    Spacer(Modifier.height(8.dp))
                    OutlinedButton(onClick = {
                        NotificationHelper.showReminder(ctx,
                            "পরীক্ষামূলক Notification!",
                            "Notification ঠিকঠাক কাজ করছে। তুমি প্রস্তুত!")
            ...
            Button(onClick = {
                scope.launch {
                    val u = repo.userFlow().value ?: return@launch
                    repo.updateSettings(u.copy(
